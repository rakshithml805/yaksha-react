...
import { Box, Typography, FormControl, Grid, Button, Checkbox,TextField} from "@mui/material";
import { useTranslation } from 'react-i18next';

export default function MultipleChoiceQuesions() {
      ...
      <>
        <Grid item xs={4}>
          <Box sx={{minHeight:'750px'}}>
            <Typography variant="h2">{t('createQuestion.multipleChoiceQuestions')}</Typography>
              <Typography variant="caption" color="text.disabled">(Enable checkbox for marking correct answers)</Typography>
              <FormControl sx={{width:'100%'}}>
                  <Box sx={{mt:2}}>
                      <Grid container rowGap={3}>
                          {fields.map((field) =>(
                              <Grid item xs={12} className="d-flex align-center" key={field.id}>
                                  <Checkbox  sx={{ '& .MuiSvgIcon-root': { fontSize: 28 } }}  id={`checkbox_${field.id}`} name={`checkbox_${field.id}`}/> 
                                  <TextField id="outlined-basic" label={field.placeholder} variant="outlined" fullWidth value={field.value} onChange={(e) => handleChange(field.id, e.target.value)}/>
                              </Grid>
                          ))}
                          <Grid item xs>
